...
const POS_SCHEMA = Yup.object().shape({
  ...
  posDescription: Yup.string().required(
    "Please provide point of sale description."
  ),
  posTypeCode: Yup.number()
    .typeError("Point of sale type code must be a number.")
    .required("Please select point of sale type."),
  posVillageCode: Yup.number()
    .typeError("Point of sale village code must be a number.")
    .required("Please select point of sale village."),
  posCampusCode: Yup.number()
    .typeError("Point of sale campus code must be a number.")
    .required("Please select point of sale campus."),
  posStatus: Yup.string()
    .oneOf(["ACTIVE", "INACTIVE"])
    .required("Please select point of sale status."),
  groupName: Yup.string().required("Please provide group name."),
  mobileNumber: Yup.string()
    .matches(phoneNumberRegex, "Please provide a valid phone number.")
    .required("Please provide group phone number."),
  email: Yup.string()
    .email("Please provide a valid email")
    .matches(emailRegex, "Please provide a valid email.")
    .required("Please provide group email address."),
  ...
  numberOfMembers: Yup.number().required(
    "Please provide number of members of the group."
  ),
...
const Pos = () => {
  ...
  const getAllPosLoading = useSelector(
    (state) => state?.pos?.loading?.getAllPoses
  );
  const poses = useSelector((state) => state?.pos?.poses);
  const addAPosLoading = useSelector((state) => state?.pos?.loading?.addAPos);
  const updateAPosLoading = useSelector(
    (state) => state?.pos?.loading?.updateAPos
  );
  const deleteAPosLoading = useSelector(
    (state) => state?.pos?.loading?.deleteAPos
  );
  const addAPosSuccess = useSelector((state) => state?.pos?.success?.addAPos);
  const updateAPosSuccess = useSelector(
    (state) => state?.pos?.success?.updateAPos
  );
  const deleteAPosSuccess = useSelector(
    (state) => state?.pos?.success?.deleteAPos
  );
  ...
  const addASignatoryLoading = useSelector(
    (state) => state?.pos?.loading?.addASignatory
  );
  const addASignatorySuccess = useSelector(
    (state) => state?.pos?.success?.addASignatory
  );
              ...
              label: (
                <div style={{ display: "flex", gap: "10px" }}>
                  <IoDocumentTextOutline
                    style={{ width: "20px", height: "20px" }}
                    className="text-grey-600 font-medium text-xl"
                  />
                  <Typography
                    style={{
                      fontSize: "14px",
          ...
          return {
            ...
            action: (
              <Dropdown menu={{ items }} trigger={["click"]}>
                ...
                >
                  <IoEllipsisVertical
                    style={{ fontSize: "28px", height: "24px", width: "24px" }}
                  />
                </div>
              </Dropdown>
            ),
  ...
  const signatoriesFormik = useFormik({
    ...
    onSubmit: (values) => {
      if (selectedPosCode) {
        dispatch(
          addASignatory({ posCode: selectedPosCode, signatoryData: values })
        );
      }
    },
  });
  ...
  return (
    <div className="font-sans">
      <div className="flex justify-between items-center mb-2">
        ...
        <div>
          <Button
            ...
            htmlType="button"
            onClick={showModal}
            className="text-sm font-semibold px-6 h-10 text-white font-sans "
          >
            + New Group
          </Button>
        </div>
      ...
      <Modal
        title={
          <h2 className="text-xl font-semibold">
            {editingPos ? "Update group details" : "Add a New Group"}
          </h2>
        }
              ...
              <div className="flex justify-start lg:gap-8">
                <div className="flex gap-4">
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      <label
                        htmlFor="posCampusCode"
                        className="text-sm font-semibold"
                      >
                        Point of Sale Campus
                      </label>
                      <Select
                        placeholder="Select Point of Sale Campus."
                        type="text"
                        name="posCampusCode"
                        id="posCampusCode"
                        filterOption={(input, option) =>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <Select
                        ...
                        className={`w-80 h-11 ${
                          formik.touched.signingOptions &&
                          formik.errors.signingOptions
                            ? "border-red-600"
                            : ""
                        }`}
                      />
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="flex flex-col gap-1">
                      <label
                        htmlFor="refereeMemberNumber"
                        className="text-sm font-semibold"
                      >
                        Referee Member Number
                      </label>
                      <Input
                        placeholder="Referee Member Number"
                        type="text"
                        name="refereeMemberNumber"
                        id="refereeMemberNumber"
                        onBlur={(e) => {
                          formik.setFieldValue(
                            "refereeMemberNumber",
                            e.target.value.charAt(0).toUpperCase() +
                    ...
                    <div className="flex flex-col gap-1">
                      <label
                        htmlFor="villageEstate"
                        className="text-sm font-semibold"
                      >
                        Village Estate
                      </label>
                      <Select
                        placeholder="Select Village Estate."
                        type="text"
                        name="villageEstate"
                        id="villageEstate"
                        filterOption={(input, option) =>
                        ...
                        className={`w-80 h-11 ${
                          formik.touched.villageEstate &&
                          formik.errors.villageEstate
                            ? "border-red-600"
                            : ""
                        }`}
                      />
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <Select
                        ...
                        onBlur={formik.handleBlur}
                        onChange={(value) =>
                          formik.setFieldValue("posStatus", value)
                        }
                        value={formik.values.posStatus}
                        className={`w-80 h-11 ${
                          formik.touched.posStatus && formik.errors.posStatus
                            ? "border-red-600"
                            : ""
                        }`}
                      />
                    ...
                    <div className="flex items-center justify-between  mt-4 ">
                      ...
                      <Button
                        type="primary"
                        htmlType="submit"
                        loading={addAPosLoading || updateAPosLoading}
                        disabled={addAPosLoading || updateAPosLoading}
                        className="w-28 text-sm font-semibold h-10 text-white font-sans"
                      >
                        {" "}
                        {editingPos ? "Update" : "Continue"}
                      </Button>
                    </div>
                  </div>
                </div>
      ...
      >
        ...
        <div className="flex items-center justify-end  mt-6  gap-8">
          ...
          <Button
            onClick={deletePos}
            type="primary"
            loading={deleteAPosLoading}
            htmlType="button"
            disabled={deleteAPosLoading}
            className="w-28 text-sm font-semibold h-10 text-white font-sans"
          >
            Delete
          </Button>
      ...
      <Modal
        width={368}
        open={openSignatoriesModal}
        footer={null}
        onCancel={handleCloseSignatoriesModal}
        title={
          <Typography
            style={{
              fontSize: "18px",
          ...
          >
            <div className="flex flex-col gap-1">
              ...
              <Input
                ...
                className={`w-80 h-11 ${
                  signatoriesFormik.touched.fullName &&
                  signatoriesFormik.errors.fullName
                    ? "border-red-600"
                    : ""
                }`}
              />
            ...
            <div className="flex flex-col gap-1">
              ...
              <Input
                ...
                className={`w-80 h-11 ${
                  signatoriesFormik.touched.idNumber &&
                  signatoriesFormik.errors.idNumber
                    ? "border-red-600"
                    : ""
                }`}
              />
            ...
            <div className="flex flex-col gap-1">
              ...
              <Input
                ...
                className={`w-80 h-11 ${
                  signatoriesFormik.touched.nationality &&
                  signatoriesFormik.errors.nationality
                    ? "border-red-600"
                    : ""
                }`}
              />
            ...
            <div className="flex flex-col gap-1">
              ...
              <Input
                ...
                className={`w-80 h-11 ${
                  signatoriesFormik.touched.designation &&
                  signatoriesFormik.errors.designation
                    ? "border-red-600"
                    : ""
                }`}
              />
            ...
            >
              <Button
                htmlType="button"
                onClick={handleCloseSignatoriesModal}
                className="w-28 text-sm font-semibold h-10 font-sans"
              >
                Cancel
              </Button>
              <Button
                loading={addASignatoryLoading}
                type="primary"
                htmlType="submit"
                className="w-28 text-sm font-semibold h-10 text-white font-sans"
              >
